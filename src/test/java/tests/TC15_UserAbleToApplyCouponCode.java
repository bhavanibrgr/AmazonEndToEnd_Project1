...
public class TC15_UserAbleToApplyCouponCode extends BaseClass{

	 Logger log=LogManager.getLogger(TC15_UserAbleToApplyCouponCode.class);
     
		@Test(dataProvider = "loginDataprovider", dataProviderClass = LoginDataProviderFromExcel.class, 
				retryAnalyzer = RetryLogic.class)
		public void couponCode(String username, String password, String type)  {
			...
			log.info("User able to provide coupon code");
			Reporter.log("User able to provide coupon code");
			
			PaymentCheckoutPage payment=new PaymentCheckoutPage(driver);
			SoftAssert sa=new SoftAssert();
			sa.assertTrue(payment.checkingCouponcodeAndApply());
			sa.assertAll();
