...
public class TC13_VerifyPaymentCheckout_OrderReview extends BaseClass{

	 Logger log=LogManager.getLogger(TC13_VerifyPaymentCheckout_OrderReview.class);
     
		@Test(dataProvider = "loginDataprovider", dataProviderClass = LoginDataProviderFromExcel.class, 
				retryAnalyzer = RetryLogic.class)
		public void verifyPaymentCheckout_OrderTotal(String username, String password, String type)  {
			...
			log.info("Order review done");
			Reporter.log("Order review done");
			
			PaymentCheckoutPage payment=new PaymentCheckoutPage(driver);
			SoftAssert sa=new SoftAssert();
			sa.assertTrue(payment.orderTotalChecking());
			sa.assertAll();
