...
public class TC11_VerifyPaymentCheckout_Address extends BaseClass{

	 Logger log=LogManager.getLogger(TC11_VerifyPaymentCheckout_Address.class);
     
		@Test(dataProvider = "loginDataprovider", dataProviderClass = LoginDataProviderFromExcel.class, 
				retryAnalyzer = RetryLogic.class)
		public void verifyPaymentCheckout(String username, String password, String type)  {
			...
			log.info("Verified payment checkout page for address");
			Reporter.log("Verified payment checkout page for address");
			
			PaymentCheckoutPage payment=new PaymentCheckoutPage(driver);
			SoftAssert sa=new SoftAssert();
			sa.assertTrue(payment.checkingAddress());
			sa.assertAll();
